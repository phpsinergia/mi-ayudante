...
EnviarFormComando:
	...
	proyectoIni := Config.Base.AppDir . "\datos\" . Config.Base.Entorno . "_" . Config.Usuario.ProyectoActual . ".ini"
...
MostrarFormComando(script, comando, args, tmenu, titulo) {
	local anchoCampoRuta, campos := [], etiquetas := [], valores := [], x := 15, y := 15, factor := 1, valor
	...
	local proyectoIni := Config.Base.AppDir . "\datos\" . Config.Base.Entorno . "_" . Config.Usuario.ProyectoActual . ".ini"
	...
	Loop % campos.MaxIndex()
	{
		...
		{
			Gui, FormComando:Add, DropDownList, x+5 y%y% w%anchoCampo% vArg_%idx%, % _DicOpcionesCampos[param]
			...
			{
				Gui, FormComando:Add, Edit, r1 x+5 y%y% w%anchoCampoRuta% vArg_%idx%, %valorGuardado%
				btnLabel := isArchivo ? "📄" : "📂"
				...
				Gui, FormComando:Add, Edit, x+5 y%y% w%anchoCampo% vArg_%idx%, %valorGuardado%
			}
			factor := (StrLen(valorGuardado) / 100) + 1
			if (factor < 1) {
				factor := 1
			}
			y += 30 * factor
...
IniciarConsolaPersistente() {
	...
	rutaTrabajo := StrReplace(rutaTrabajo, "[Proyecto]", Config.Usuario.ProyectoActual)
	if !WinExist(titulo) {
